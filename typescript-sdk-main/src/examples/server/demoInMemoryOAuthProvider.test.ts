...
describe("DemoInMemoryAuthProvider", () => {
  ...
  const createMockResponse = (): Response & {
    ...
    const mockRedirect = jest.fn().mockImplementation(
      (url: string | number, status?: number) => {
    ...
    const mockResponse = {
      ...
      getRedirectUrl: () => {
        if (capturedRedirectUrl === undefined) {
          throw new Error(
            "No redirect URL was captured. Ensure redirect() was called first.",
          );
        }
  ...
  describe("authorize", () => {
    ...
    it("should throw InvalidRequestError for unregistered redirect_uri", async () => {
      ...
      await expect(provider.authorize(validClient, params, mockResponse))
  ...
  describe("challengeForAuthorizationCode", () => {
    ...
    it("should return the code challenge for a valid authorization code", async () => {
      ...
      await provider.authorize(validClient, params, mockResponse);
      const code = new URL(mockResponse.getRedirectUrl()).searchParams.get(
        "code",
      )!;
  ...
  describe("exchangeAuthorizationCode", () => {
    ...
    it("should throw error when client_id does not match", async () => {
      ...
      await expect(provider.exchangeAuthorizationCode(differentClient, code))
        .rejects.toThrow(
    ...
    it("should delete authorization code after successful exchange", async () => {
      ...
      await expect(provider.exchangeAuthorizationCode(validClient, code))
        .rejects.toThrow("Invalid authorization code");
    });
    ...
    it("should validate resource when validateResource is provided", async () => {
      ...
      await expect(strictProvider.exchangeAuthorizationCode(validClient, code))
        .rejects.toThrow(
