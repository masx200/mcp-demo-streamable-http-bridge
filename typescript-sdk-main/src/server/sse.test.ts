...
const createMockRequest = (
  ...
  const mockReq = {
    ...
    on: jest.fn<http.IncomingMessage["on"]>().mockImplementation(
      (event, listener) => {
        const mockListener = listener as unknown as (
          ...args: unknown[]
        ) => void;
        if (event === "data") {
 ...
 */
async function createTestServerWithSse(
  args: { mockRes: http.ServerResponse },
): Promise<{
  server: Server;
...
describe("SSEServerTransport", () => {
  ...
  describe("start method", () => {
    ...
    it("should pass request info to tool callback", async () => {
      const mockRes = createMockResponse();
      const { mcpServer, baseUrl, sessionId, serverPort } =
        await createTestServerWithSse({ mockRes });
      ...
      const response = await sendSsePostRequest(
        baseUrl,
        toolCallMessage,
        sessionId,
      );
  ...
  describe("handlePostMessage method", () => {
    it("should return 500 if server has not started", async () => {
      ...
      await expect(transport.handlePostMessage(mockReq, mockRes)).rejects
        .toThrow(error);
    ...
    it("should return 400 if content-type is not application/json", async () => {
      ...
      await expect(transport.handlePostMessage(mockReq, mockRes)).resolves.toBe(
        undefined,
      );
  ...
  describe("DNS rebinding protection", () => {
    ...
    describe("Host header validation", () => {
      it("should accept requests with allowed host headers", async () => {
        ...
        await transport.handlePostMessage(mockReq, mockHandleRes, {
          jsonrpc: "2.0",
          method: "test",
        });
      ...
      it("should reject requests with disallowed host headers", async () => {
        ...
        expect(mockHandleRes.end).toHaveBeenCalledWith(
    ...
    describe("Combined validations", () => {
      it("should validate both host and origin when both are configured", async () => {
        ...
        await transport.handlePostMessage(mockReq1, mockHandleRes1, {
          jsonrpc: "2.0",
          method: "test",
        });

        expect(mockHandleRes1.writeHead).toHaveBeenCalledWith(403);
        expect(mockHandleRes1.end).toHaveBeenCalledWith(
          "Invalid Origin header: http://evil.com",
        );
        ...
        await transport.handlePostMessage(mockReq2, mockHandleRes2, {
          jsonrpc: "2.0",
          method: "test",
        });

        expect(mockHandleRes2.writeHead).toHaveBeenCalledWith(403);
        expect(mockHandleRes2.end).toHaveBeenCalledWith(
          "Invalid Host header: evil.com",
        );
        ...
        await transport.handlePostMessage(mockReq3, mockHandleRes3, {
          jsonrpc: "2.0",
          method: "test",
        });
