...
describe("Token Handler", () => {
  ...
  describe("Authorization code grant", () => {
    ...
    it("returns tokens for valid code exchange", async () => {
      const mockExchangeCode = jest.spyOn(
        mockProvider,
    ...
    it("passes through code verifier when using proxy provider", async () => {
      ...
      try {
        ...
        const proxyProvider = new ProxyOAuthServerProvider({
          ...
          getClient: async (
            clientId,
          ) => (clientId === "valid-client" ? validClient : undefined),
        });
        ...
        const response = await supertest(proxyApp).post("/token").type("form")
          .send({
            client_id: "valid-client",
            client_secret: "valid-secret",
            grant_type: "authorization_code",
  ...
  describe("Refresh token grant", () => {
    ...
    it("returns new tokens for valid refresh token", async () => {
      const mockExchangeRefresh = jest.spyOn(
        mockProvider,
  ...
  describe("CORS support", () => {
    it("includes CORS headers in response", async () => {
      const response = await supertest(app).post("/token").type("form").set(
        "Origin",
        "https://example.com",
      ).send({
        client_id: "valid-client",
