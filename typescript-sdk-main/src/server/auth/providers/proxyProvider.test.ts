...
describe("Proxy OAuth Server Provider", () => {
  ...
  describe("client registration", () => {
    ...
    it("handles registration failure", async () => {
      ...
      await expect(provider.clientsStore.registerClient!(newClient)).rejects
        .toThrow(ServerError);
    });
  });
  ...
  describe("token verification", () => {
    ...
    it("passes through InvalidTokenError", async () => {
      ...
      await expect(provider.verifyAccessToken("invalid-token")).rejects.toBe(
        error,
      );
    ...
    it("passes through unexpected errors", async () => {
      ...
      await expect(provider.verifyAccessToken("valid-token")).rejects.toBe(
        error,
      );
      expect(mockVerifyToken).toHaveBeenCalledWith("valid-token");
    });
