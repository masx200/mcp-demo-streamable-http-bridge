...
describe("requireBearerAuth middleware", () => {
  ...
  it.each([
    ...
    "should reject expired tokens (expired %s seconds ago)",
    async (expiredSecondsAgo: number) => {
  ...
  it.each([
    ...
    "should reject tokens with no expiration time (expiresAt: %s)",
    async (expiresAt: number | undefined) => {
  ...
  it("should return 400 for generic OAuthError", async () => {
    ...
    mockVerifyAccessToken.mockRejectedValue(
      new CustomOAuthError("custom_error", "Some OAuth error"),
    );
  ...
  describe("with resourceMetadataUrl", () => {
    ...
    it("should include resource_metadata in WWW-Authenticate header for insufficient scope errors", async () => {
      ...
      mockVerifyAccessToken.mockRejectedValue(
        new InsufficientScopeError("Required scopes: admin"),
      );
