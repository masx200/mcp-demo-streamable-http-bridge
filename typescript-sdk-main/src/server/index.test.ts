...
test("should respect server notification capabilities", async () => {
  ...
  const [_clientTransport, serverTransport] = InMemoryTransport
    .createLinkedPair();
  ...
  await expect(server.sendResourceUpdated({ uri: "test://resource" })).rejects
    .toThrow(/^Server does not support/);
});
