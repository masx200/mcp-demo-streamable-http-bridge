...
> {
  ...
  constructor(
    ...
    if (this._capabilities.logging) {
      this.setRequestHandler(SetLevelRequestSchema, async (request, extra) => {
        const transportSessionId: string | undefined = extra.sessionId ||
          (extra.requestInfo?.headers["mcp-session-id"] as string) || undefined;
  ...
  async elicitInput(
    ...
    if (
      result.action === "accept" && result.content && params.requestedSchema
    ) {
      try {
