...
type RemovePassthrough<T> = T extends object
  ? T extends Array<infer U> ? Array<RemovePassthrough<U>>
...
type MakeUnknownsNotOptional<T> = IsUnknown<T> extends true ? unknown
  : T extends object
    ? T extends Array<infer U> ? Array<MakeUnknownsNotOptional<U>>
    : T extends Function ? T
    :
      & Pick<T, never>
      & {
        ...
        [
          K in keyof T as "_meta" extends K ? never
            : IsUnknown<T[K]> extends true ? K
            : never
        ]-?: unknown;
      }
      & Pick<
        T,
        {
          ...
          [K in keyof T]: "_meta" extends K ? K
            : IsUnknown<T[K]> extends true ? never
            : K;
        }[keyof T]
      >
      & {
...
function extractExportedTypes(source: string): string[] {
  return [...source.matchAll(/export\s+(?:interface|class|type)\s+(\w+)\b/g)]
    .map((m) => m[1]);
}
...
describe("Spec Types", () => {
  ...
  const typesToCheck = specTypes.filter((type) =>
    !MISSING_SDK_TYPES.includes(type)
  );
  ...
  describe("Missing SDK Types", () => {
    it.each(MISSING_SDK_TYPES)(
      ...
      (type) => {
        expect(sdkTypeChecks[type as keyof typeof sdkTypeChecks])
          .toBeUndefined();
      },
    );
  });
