...
describe("OAuth Authorization", () => {
  ...
  describe("discoverOAuthMetadata", () => {
    ...
    it("returns metadata when discovery succeeds with path", async () => {
      ...
      const metadata = await discoverOAuthMetadata(
        "https://auth.example.com/path/name",
      );
      expect(metadata).toEqual(validMetadata);
      const calls = mockFetch.mock.calls;
      expect(calls.length).toBe(1);
    ...
    it("falls back when path-aware discovery encounters CORS error", async () => {
      ...
      const metadata = await discoverOAuthMetadata(
        "https://auth.example.com/deep/path",
      );
      expect(metadata).toEqual(validMetadata);
    ...
    it("throws an error when all fetch attempts fail", async () => {
      ...
      // Should fail with the second error
      await expect(discoverOAuthMetadata("https://auth.example.com")).rejects
  ...
  describe("buildDiscoveryUrls", () => {
    it("generates correct URLs for server without path", () => {
      ...
      expect(urls.map((u) => ({ url: u.url.toString(), type: u.type })))
        .toEqual([
          {
            url:
              "https://auth.example.com/.well-known/oauth-authorization-server",
  ...
  describe("startAuthorization", () => {
    ...
    it.each([validMetadata, validOpenIdMetadata])(
      "uses metadata authorization_endpoint when provided",
      async (baseMetadata) => {
        const { authorizationUrl } = await startAuthorization(
          "https://auth.example.com",
          {
    ...
    it.each([validMetadata, validOpenIdMetadata])(
      "validates response type support",
      async (baseMetadata) => {
        const metadata = {
          ...baseMetadata,
  ...
  describe("registerClient", () => {
    ...
    it("throws on error response", async () => {
      mockFetch.mockResolvedValueOnce(
        Response.json(
          new ServerError("Dynamic client registration failed")
            .toResponseObject(),
          { status: 400 },
        ),
      );
  ...
  describe("auth function", () => {
    ...
    it("passes resource parameter through authorization flow", async () => {
      ...
      const redirectCall =
        (mockProvider.redirectToAuthorization as jest.Mock).mock.calls[0];
    ...
    it("includes resource in token exchange when authorization code is provided", async () => {
      ...
      (mockProvider.codeVerifier as jest.Mock).mockResolvedValue(
        "test-verifier",
      );
    ...
    it("skips default PRM resource validation when custom validateResourceURL is provided", async () => {
      ...
      // Mock provider methods
      (providerWithCustomValidation.clientInformation as jest.Mock)
        .mockResolvedValue({
          client_id: "test-client",
          client_secret: "test-secret",
        });
      (providerWithCustomValidation.tokens as jest.Mock).mockResolvedValue(
        undefined,
      );
      (providerWithCustomValidation.saveCodeVerifier as jest.Mock)
        .mockResolvedValue(undefined);
      (providerWithCustomValidation.redirectToAuthorization as jest.Mock)
        .mockResolvedValue(undefined);
