...
describe("StreamableHTTPClientTransport", () => {
  ...
  it("should store session ID received during initialization", async () => {
    ...
    await transport.send(
      { jsonrpc: "2.0", method: "test", params: {} } as JSONRPCMessage,
    );
  ...
  it("should support custom reconnection options", () => {
    ...
    expect(transportInstance._reconnectionOptions.initialReconnectionDelay)
      .toBe(500);
  ...
  it("uses custom fetch implementation if provided", async () => {
    ...
    await transport.send(
      { jsonrpc: "2.0", method: "test", params: {}, id: "1" } as JSONRPCMessage,
    );
  ...
  it("invalidates all credentials on UnauthorizedClientError during auth", async () => {
    ...
    (global.fetch as jest.Mock)
      ...
      .mockResolvedValueOnce(
        Response.json(
          new UnauthorizedClientError("Client not authorized")
            .toResponseObject(),
          { status: 400 },
        ),
      )
