...
describe("withOAuth", () => {
  ...
  it("should add Authorization header when tokens are available (with explicit baseUrl)", async () => {
    ...
    const enhancedFetch = withOAuth(mockProvider, "https://api.example.com")(
      mockFetch,
    );
  ...
  it("should retry request after successful auth on 401 response (with explicit baseUrl)", async () => {
    ...
    mockFetch.mockResolvedValueOnce(unauthorizedResponse).mockResolvedValueOnce(
      successResponse,
    );
...
describe("withLogging", () => {
  ...
  it("should respect statusLevel option", async () => {
    ...
    mockFetch.mockResolvedValueOnce(successResponse).mockResolvedValueOnce(
      errorResponse,
    );
...
describe("createMiddleware", () => {
  ...
  it("should support conditional middleware logic", async () => {
    ...
    mockFetch.mockResolvedValueOnce(apiResponse).mockResolvedValueOnce(
      publicResponse,
    );
  ...
  it("should support short-circuit responses", async () => {
    ...
    const normalResponse = await enhancedFetch(
      "https://example.com/normal/data",
    );
