          ...
          res.end(
            JSON.stringify({
              issuer: `http://127.0.0.1:${
                (authServer.address() as AddressInfo).port
              }`,
              authorization_endpoint: `http://127.0.0.1:${
                (authServer.address() as AddressInfo).port
              }/authorize`,
              token_endpoint: `http://127.0.0.1:${
                (authServer.address() as AddressInfo).port
              }/token`,
              registration_endpoint: `http://127.0.0.1:${
                (authServer.address() as AddressInfo).port
              }/register`,
              response_types_supported: ["code"],
              code_challenge_methods_supported: ["S256"],
    ...
    it("uses custom fetch during auth flow on SSE connection 401 - no global fetch fallback", async () => {
      ...
      resourceServerHandler.mockImplementation((req, res) => {
        if (req.url === "/") {
          ...
          res.writeHead(401, {
            "WWW-Authenticate":
              `Bearer realm="mcp", resource_metadata="${resourceBaseUrl.href}.well-known/oauth-protected-resource"`,
          });
          res.end();
          return;
        }
