...
describe("SafeUrlSchema", () => {
  it("accepts valid HTTPS URLs", () => {
    expect(SafeUrlSchema.parse("https://example.com")).toBe(
      "https://example.com",
    );
    expect(SafeUrlSchema.parse("https://auth.example.com/oauth/authorize"))
      .toBe("https://auth.example.com/oauth/authorize");
  });

  it("accepts valid HTTP URLs", () => {
    expect(SafeUrlSchema.parse("http://localhost:3000")).toBe(
      "http://localhost:3000",
    );
  });
...
describe("OAuthMetadataSchema", () => {
  ...
  it("rejects metadata with javascript: URLs", () => {
    ...
    expect(() => OAuthMetadataSchema.parse(metadata)).toThrow(
      "URL cannot use javascript:, data:, or vbscript: scheme",
    );
  });
...
describe("OpenIdProviderMetadataSchema", () => {
  ...
  it("rejects metadata with javascript: in jwks_uri", () => {
    ...
    expect(() => OpenIdProviderMetadataSchema.parse(metadata)).toThrow(
      "URL cannot use javascript:, data:, or vbscript: scheme",
    );
  });
});
...
describe("OAuthClientMetadataSchema", () => {
  ...
  it("rejects client metadata with javascript: redirect URIs", () => {
    ...
    expect(() => OAuthClientMetadataSchema.parse(metadata)).toThrow(
      "URL cannot use javascript:, data:, or vbscript: scheme",
    );
  });
});
